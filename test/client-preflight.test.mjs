...
const fixture = body => `window.__ModuleLoader__.load({ id: 'darask-harness', factory: () => ({ apply(ctx) { ${body} } }) });`;
...
test('missing apply cannot silently pass validation', () => {
  assert.throws(() => validateClientBundle(`window.__ModuleLoader__.load({ id: 'darask-harness', factory: () => ({}) });`), /export apply/);
});
