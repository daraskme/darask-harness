...
const PACKAGE = { name: 'darask-harness', version: '9.9.9-test.1' };
...
async function fixtureRoot(t, { devMode = false, layout = true } = {}) {
  ...
  if (layout) {
    ...
    await writeFile(join(root, 'app', 'package.json'), JSON.stringify({ name: 'darask-harness-app', dependencies: { 'darask-harness': 'file:../packages/darask-harness-0.0.0.tgz' } }));
  }
...
test('host-update stages the archive, launches the detached updater and exits', async t => {
  ...
  assert.ok(staged.includes('darask-harness-9.9.9-test.1.tgz'));
...
test('host-update rejects wrong host, bad payloads and dev mode', async t => {
  ...
  // darask-harness ではないパッケージ
  assert.equal((await post(routes(), { body: fixtureTgz({ name: 'evil', version: '1.0.0' }) })).status, 400);
  // バージョン形式が不正
  assert.equal((await post(routes(), { body: fixtureTgz({ name: 'darask-harness', version: 'latest' }) })).status, 400);
...
test('distributeToNodes updates dev-mode PCs, skips identical archives and pushes changed content at the same version', async t => {
  ...
  const archive = { name: 'darask-harness-9.9.9-test.1.tgz', bytes, sha256: digest, version: '9.9.9-test.1' };
...
test('installed archive preserves the exact bytes and digest used by the canonical installer', async t => {
  ...
  await writeFile(join(root, 'packages', 'darask-harness-0.0.0.tgz'), bytes);
