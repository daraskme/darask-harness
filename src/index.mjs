...
export async function apply(ctx, config) {
  ...
  if (config.syncPresets) {
    ...
    for (const id of await listShippedPresets()) {
      try {
        ...
        if (result.status === 'user-owned' || result.status === 'conflict') ctx.logger.warn(`darask-harness: preset "${id}" は ${result.target} に既に存在するため上書きしませんでした。`);
        else if (result.status !== 'unchanged') ctx.logger.info(`darask-harness: preset "${id}" を ${result.target} に${result.status === 'created' ? '作成' : '更新'}しました。`);
      } catch (error) { ctx.logger.warn(`darask-harness: preset "${id}" を配置できませんでした: ${error.message}`); }
    }
  }
  let serviceRef;
  ...
  ctx.inject(['webServer'], webCtx => {
    ...
    }, tls)), 'darask-harness: OpenRouter callback');
  });
  ...
  ctx.effect(() => () => codexProxy.dispose(), 'darask-harness Codex accounts');
  ...
  ctx.inject(['workspaceRegistry', 'connection', 'credentials'], async scope => {
    ...
    developmentHost.distribution.resume().catch(error => scope.logger.warn(`darask-harness: 配布を再開できませんでした。${error.message}`));
  ...
  ctx.effect(() => () => service.dispose(), 'darask-harness lifecycle');
}
