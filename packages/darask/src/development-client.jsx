...
export function DevelopmentPanel() {
  ...
  async function register() {
    ...
    try {
      ...
      const result = await response.json(); if (!response.ok || result.error) throw new Error(result.error || '登録できませんでした。'); setNotice('ワークスペースに登録しました。左側の一覧から dsh-darask を開いてください。');
    } catch (e) { setError(e.message); } finally { setPending(false); }
  }
  return <section className="darask"><header className="darask-heading"><div><h2>開発・更新</h2><p>Git で管理したソースを編集し、画面を再読み込みして反映します。</p></div></header>
    ...
    {data?.available && <>
      <article className="darask-provider"><header className="darask-provider-header"><h3>dsh-darask</h3><Tag>{data.version}</Tag><Tag>{data.branch || 'ブランチ未選択'}</Tag><Tag>{data.dirty ? '未コミットの編集あり' : '変更なし'}</Tag></header><div className="darask-usage">
      ...
      {data?.distribute && <article className="darask-provider"><header className="darask-provider-header"><h3>登録済みの PC に配布</h3><Tag>{data.distribute.nodes} 台</Tag></header><div className="darask-usage">
        <p className="darask-muted">この PC の dsh-darask を登録済みの PC に配布します。各 PC は DSH を再起動するため、実行中の作業を保存してください。開発モードの PC は GitHub から更新します。</p>
