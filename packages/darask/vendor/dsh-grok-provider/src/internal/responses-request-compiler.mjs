...
async function compileImageRequest({
  ...
  await Promise.all([...refs.entries()].map(async ([attachmentId, ref]) => {
    ...
    signal?.throwIfAborted()
    let version = captureRequestImage(returned, policy)
    validateRequestImage(version, ref, { ...policy, mediaTypes: CAPTURE_IMAGE_POLICY.mediaTypes })
    version = await encodeWireImage(version, policy, signal)
...
function placeholderRequestImages(blocks) {
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    ...
    images.set(
      block.attachment,
      Object.freeze({ mediaType: "image/png", data: EMPTY_REQUEST_IMAGE }),
    )
  }
  return images
}
...
async function encodeWireImage(version, policy, signal) {
  if (policy.mediaTypes.includes(version.mediaType)) return version
  // DSH normalizes transparent images to WebP, while Grok accepts JPEG/PNG.
  // Project only the request bytes, retaining the immutable attachment identity.
  let width = version.width, height = version.height
  for (;;) {
    signal?.throwIfAborted()
    const { data, info } = await sharp(version.data, { limitInputPixels: policy.readPolicy.maxPixels })
      .resize({ width, height, fit: "inside", withoutEnlargement: true }).toColourspace("srgb").png().toBuffer({ resolveWithObject: true })
    signal?.throwIfAborted()
    if (data.byteLength <= policy.readPolicy.maxBytes) return { ...version, data, mediaType: "image/png", bytes: data.byteLength, width: info.width, height: info.height }
    if (width === 1 && height === 1) throw new InvalidRequestImageProjectionError()
    width = Math.max(1, Math.floor(width * 0.75)); height = Math.max(1, Math.floor(height * 0.75))
  }
}

function hasExpectedMagic(data, mediaType) {
  if (mediaType === "image/webp") return data.length >= 12 && Buffer.from(data.subarray(0, 4)).toString() === "RIFF" && Buffer.from(data.subarray(8, 12)).toString() === "WEBP"
