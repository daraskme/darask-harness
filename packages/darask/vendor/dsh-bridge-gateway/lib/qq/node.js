...
export class QqConversationNode extends ConversationBridge {
  ...
  async sendText(text) {
    ...
    // 检测是否是提示用户开始新会话的消息
    const isPromptMessage = text.includes('アクティブな会話はありません') || text.includes('会話の復元に失敗しました')
          ...
          rows: [
            {
              buttons: [
                {
                  id: 'new_conversation',
                  render_data: { label: '🆕 新しい会話', visited_label: '新しい会話', style: 1 },
                  action: { type: 1, permission: { type: 2 }, data: 'new', unsupport_tips: 'QQクライアントを更新してください' },
                },
                {
                  id: 'list_sessions',
                  render_data: { label: '📋 会話一覧', visited_label: '会話一覧', style: 1 },
                  action: { type: 1, permission: { type: 2 }, data: 'list', unsupport_tips: 'QQクライアントを更新してください' },
                },
              ],
            },
            {
              buttons: [
                {
                  id: 'help',
                  render_data: { label: '❓ ヘルプ', visited_label: 'ヘルプ', style: 1 },
                  action: { type: 1, permission: { type: 2 }, data: 'help', unsupport_tips: 'QQクライアントを更新してください' },
                },
              ],
            },
          ],
    ...
    const isApprovalMessage = text.includes('操作の許可確認')
          ...
          rows: [
            {
              buttons: [
                {
                  id: 'approve_yes',
                  render_data: { label: '✓ 承認して実行', visited_label: '承認済み', style: 1 },
                  action: { type: 1, permission: { type: 2 }, data: '/yes', unsupport_tips: 'QQクライアントを更新してください' },
                },
                {
                  id: 'approve_no',
                  render_data: { label: '✕ 拒否する', visited_label: '拒否済み', style: 0 },
                  action: { type: 1, permission: { type: 2 }, data: '/no', unsupport_tips: 'QQクライアントを更新してください' },
                },
              ],
            },
          ],
    ...
    const isResolutionMessage = text.includes('実行を承認しました') || text.includes('実行を拒否しました')
