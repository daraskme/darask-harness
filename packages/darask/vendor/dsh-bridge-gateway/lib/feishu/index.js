...
export class FeishuService extends Platform {
  ...
  async start() {
    if (!this.gateway.configured) {
      this.setStatus('idle')
      return { success: false, error: 'App ID と App Secret が未設定です' }
    }
    ...
    if (ok) {
      ...
      this.setStatus('error', this.gateway.error || '接続に失敗しました')
  ...
  async login(creds = {}) {
    ...
    if (!this.gateway.configured) {
      await this.stop()
      return { success: false, error: 'App ID と App Secret をすべて入力してください' }
    }
