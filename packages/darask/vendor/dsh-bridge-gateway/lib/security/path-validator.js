...
export async function isSafeWorkspacePath(targetPath, options = {}) {
  if (!targetPath || typeof targetPath !== 'string') {
    return { valid: false, error: 'パスを空にできません' };
  }

  const trimmed = targetPath.trim();
  if (!trimmed || trimmed.includes('\0')) {
    return { valid: false, error: '不正なパス文字 (Null byte)' };
  }
  ...
  try {
    ...
    return { valid: false, error: `パスの解析に失敗しました: ${err.message}` };
  }
  ...
  if (winDriveMatch) {
    ...
    for (const prefix of WIN_SYSTEM_PREFIXES) {
      if (rest === prefix || rest.startsWith(`${prefix}\\`) || rest.startsWith(`${prefix}/`)) {
        return { valid: false, error: `セキュリティによりブロック：システムの機密ディレクトリにはアクセスできません (${prefix})` };
      }
    }
  }
  ...
  if (isWin) {
    ...
    if (driveMatch) {
      ...
      return { valid: false, error: 'セキュリティによりブロック：UNC ネットワーク共有パスにはアクセスできません' };
    }
  } else {
    // 3. 检查 POSIX 敏感路径
    for (const prefix of POSIX_SYSTEM_PREFIXES) {
      if (lowerNormalized === prefix || lowerNormalized.startsWith(`${prefix}/`)) {
        return { valid: false, error: `セキュリティによりブロック：システムの重要ディレクトリにはアクセスできません (${prefix})` };
      }
    }
  }
  ...
  for (const part of pathParts) {
    if (isSensitiveFolderName(part)) {
      return { valid: false, error: `セキュリティによりブロック：機密設定ディレクトリ「${part}」にはアクセスできません` };
    }
  }
  ...
  try {
    ...
    if (!s.isDirectory()) {
      return { valid: false, error: `指定パスはフォルダではありません: ${normalized}` };
    }
    ...
    return { valid: false, error: `指定パスにアクセスできません (${err.message})` };
  }
  ...
  if (realTargetPath && realTargetPath !== normalized) {
    ...
    if (isWin) {
      ...
      if (realDriveMatch) {
        ...
        for (const prefix of WIN_SYSTEM_PREFIXES) {
          if (rest === prefix || rest.startsWith(`${prefix}\\`)) {
            return { valid: false, error: `セキュリティによりブロック：シンボリックリンクがシステムの機密ディレクトリを指しています (${prefix})` };
          }
        }
      }
    } else {
      for (const prefix of POSIX_SYSTEM_PREFIXES) {
        if (realLower === prefix || realLower.startsWith(`${prefix}/`)) {
          return { valid: false, error: `セキュリティによりブロック：シンボリックリンクがシステムの重要ディレクトリを指しています (${prefix})` };
        }
      }
    }
    ...
    for (const part of realParts) {
      if (isSensitiveFolderName(part)) {
        return { valid: false, error: `セキュリティによりブロック：シンボリックリンクが機密設定ディレクトリ「${part}」を指しています` };
      }
    }
  }
