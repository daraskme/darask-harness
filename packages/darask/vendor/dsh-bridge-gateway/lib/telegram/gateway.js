...
export class TelegramGateway extends Service {
  ...
  async registerCommands() {
    const commands = [
      { command: 'new', description: '新しい会話を開始 (/new <プロンプト>)' },
      { command: 'sessions', description: '会話一覧と切り替え' },
      { command: 'use', description: 'アクティブな会話を切替 (/use <N>)' },
      { command: 'workspaces', description: '利用可能なワークスペース一覧' },
      { command: 'status', description: 'Agent の状態と会話の要約' },
      { command: 'stop', description: '実行中のタスクを停止' },
      { command: 'end', description: '現在の会話を終了' },
      { command: 'help', description: 'ショートカットとヘルプ' },
    ]
    try {
      // 1. 设置全局默认范围
