...
export class TelegramConversationNode extends ConversationBridge {
    ...
    if (match) {
      ...
      if (pending) {
        ...
        await this.gateway?.answerCallbackQuery(
          queryId,
          action === 'approve' ? '✓ 操作を承認しました' : '✕ 操作を拒否しました',
          false,
        )
      } else {
        await this.gateway?.answerCallbackQuery(queryId, '⚠️ この承認は処理済みか期限切れです', false)
      }
      return
    }
  ...
  async sendApprovalCard(approvalId, request) {
    ...
    const toolName = request?.name || request?.tool || 'システム操作'
    const desc = request?.description || request?.summary || ''
    const command = request?.command || request?.cmd || ''

    const lines = [
      `⚠️ **操作の許可確認** (ID: <code>${approvalId}</code>)`,
      '',
      `• **ツール**：<code>${toolName}</code>`,
    ]
    if (desc) lines.push(`• **説明**：${desc}`)
    if (command) lines.push(`• **コマンド**：<code>${command}</code>`)
    lines.push('', '承認結果を選んでください（<code>1</code> で承認、<code>2</code> で拒否も可）：')

    const buttons = [
      [
        { text: '✓ 承認して実行', callback_data: `approve:${approvalId}` },
        { text: '✕ 拒否する', callback_data: `reject:${approvalId}` },
      ],
    ]
  ...
  async sendText(text) {
    ...
    if (content.includes('コマンドヘルプ') || content.includes('/new <プロンプト>')) {
      const buttons = [
        [
          { text: '📋 会話一覧', callback_data: 'cmd:sessions' },
          { text: '📁 ワークスペース', callback_data: 'cmd:workspaces' },
        ],
        [
          { text: '📊 実行状態', callback_data: 'cmd:status' },
          { text: '⏹ タスク停止', callback_data: 'cmd:stop' },
        ],
      ]
      return this.gateway.sendKeyboard(peerId, content, buttons)
    }

    if (content.includes('Agent ステータス') || content.includes('アクティブな会話はありません')) {
      const buttons = [
        [
          { text: '🔄 状態を更新', callback_data: 'cmd:status' },
          { text: '⏹ タスク停止', callback_data: 'cmd:stop' },
        ],
        [
          { text: '📋 会話一覧', callback_data: 'cmd:sessions' },
          { text: '🚪 会話を終了', callback_data: 'cmd:end' },
        ],
      ]
      return this.gateway.sendKeyboard(peerId, content, buttons)
    }

    if (content.includes('会話一覧') || content.includes('利用可能な会話')) {
      const buttons = [
        [
          { text: '📁 ワークスペース', callback_data: 'cmd:workspaces' },
          { text: '📊 状態を見る', callback_data: 'cmd:status' },
        ],
      ]
      return this.gateway.sendKeyboard(peerId, content, buttons)
    }
