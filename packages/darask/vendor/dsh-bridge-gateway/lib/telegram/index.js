...
export class TelegramService extends Platform {
  ...
  async start() {
    if (!this.gateway.configured) {
      this.setStatus('idle')
      return { success: false, error: 'Telegram Bot Token が未設定です' }
    }
    ...
    if (ok) {
      ...
      this.setStatus('error', 'Telegram API への接続に失敗しました。Bot Token またはプロキシを確認してください')
      return { success: false, error: '接続に失敗しました' }
    }
  }
  ...
  async login(creds = {}) {
    ...
    if (!this.gateway.configured) {
      await this.stop()
      return { success: false, error: 'Telegram Bot Token を入力してください' }
    }
  ...
  getStatus() {
    ...
    return {
      ...
      accountId: this.gateway.accountId || (this.gateway.configured ? 'Token 設定済み' : ''),
      allowFrom,
