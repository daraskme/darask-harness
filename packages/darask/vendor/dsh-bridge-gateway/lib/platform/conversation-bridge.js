...
function digestLine(session) {
  ...
  const steps = tools > 0 ? `${tools} 回のツール呼び出し` : '思考中'
  const last = lastTool ? ` | 直近: ${lastTool}` : ''
  return `[処理中] 第 ${turn} ターン | ${steps}${last}`
}
...
export class ConversationBridge {
  ...
  async createSession(prompt, cwdOverride) {
    ...
    try {
      ...
      if (prompt) {
        const platformName = this.platform?.name || 'IMクライアント'
        const promptWithContext = `${prompt}\n\n<!-- [dsh-bridge 提示] ユーザーは【${platformName}】経由で会話しています。ファイル/画像/レポート/スクリプトの送信・書き出しを明示的に求められた場合のみ、ローカルで用意したあと本文に次の指示を付けてください：\n[SEND_FILE: <ローカルファイルの絶対パス>]\n例：[SEND_FILE: C:\\path\\to\\report.xlsx]\nゲートウェイがこの指示を解釈し、ファイルをチャットへ直接送り、本文からは指示を取り除きます。通常のコード作成やテキスト返信、送信要求がないときはこの指示を出さないでください。 -->`
      ...
      const wsDetail = cwd ? `\n- **ワークスペース**：\`${cwd}\`` : ''
      const hint = prompt ? '' : '\n\n> 💡 任意のメッセージを送ると Agent と会話できます。'
      await this.sendText(`✓ **新しい会話を作成しました**\n- **会話 ID**：\`${fmtSessionId(handle.agent.session.id)}\`${wsDetail}${hint}`)
    } catch (error) {
      await this.sendText(`❌ **会話の作成に失敗しました**：${error instanceof Error ? error.message : String(error)}`)
    }
  }
  ...
  async handleInbound({ senderId, text, isGroup = false }) {
    try {
        ...
        if (this._restoringSessionMap.has(sessionId)) {
          ...
          const restorePromise = (async () => {
            ...
            try {
              ...
              await this.sendText(`❌ **会話の復元に失敗しました**：${reason}\n\n> \`/new <プロンプト>\` で新しい会話を作成できます。`)
              return null
            }
          })().finally(() => {
      ...
      if (!agent) {
        await this.sendText(`> 💤 **アクティブな会話はありません**\n> \`/new <プロンプト>\` で開始するか、\`/sessions\` で既存の会話を確認してください。`)
        return 'routed'
      }

      // 针对微信/IM客户端用户，注入上下文提示，规范 Agent 仅在需要向用户发送文件附件时输出 [SEND_FILE: <文件绝对路径>]
      const promptWithContext = `${fullText}\n\n<!-- [dsh-bridge 提示] ユーザーは【${this.platform.name}】経由で会話しています。ファイル/画像/レポート/スクリプトの送信・書き出しを明示的に求められた場合のみ、ローカルで用意したあと本文に次の指示を付けてください：\n[SEND_FILE: <ローカルファイルの絶対パス>]\n例：[SEND_FILE: C:\\path\\to\\report.xlsx]\nゲートウェイがこの指示を解釈し、ファイルをチャットへ直接送り、本文からは指示を取り除きます。通常のコード作成やテキスト返信、送信要求がないときはこの指示を出さないでください。 -->`
      ...
      await this.sendText(`❌ **実行エラー**：${errMsg}`).catch(() => {})
      return 'ignored'
    }
  }
    ...
    const onEvent = async (session, event) => {
      ...
      if (event.type === 'turn/end') {
        ...
        if (reason.kind === 'error') {
          void this.sendText(`❌ **処理エラー**：${summarizeError(reason.error)}`)
        } else if (reason.kind === 'aborted') {
          void this.sendText(`⏹ **タスクを停止しました**`)
        } else if (reason.kind === 'max-tokens') {
          void this.sendText(`⚠️ **モデルの 1 ターン出力上限に達したため、内容を切り詰めました**`)
        }
  ...
  _attachApprovalBridge() {
    const listener = async (req, next) => {
      ...
      const prompt = [
        ...
        '| :--- | :--- |',
        `| **ツール** | \`${req.toolName}\` |`,
        ...(req.reason ? [`| **理由** | ${String(req.reason).replace(/\|/g, '｜')} |`] : []),
        `| **タイムアウト** | ${timeoutMin} 分（時間切れで自動拒否） |`,
        '',
        `> \`/yes\`（または \`1\`）で承認`,
        `> \`/no\`（または \`2\`）で拒否`,
      ].join('\n')
      ...
      if (!timeoutFired) {
        const sourceHint = winner === 'web' ? '（Web 側の操作）' : ''
        const label = outcome === 'allowed-once' ? `✓ **実行を承認しました**${sourceHint}` : outcome === 'rejected' ? `❌ **実行を拒否しました**${sourceHint}` : `**[${outcome}]**`
...
async function validateWorkspacePath(node, sel) {
  ...
  if (!allowedPaths.includes(normalized)) {
    ...
    const more = allowedPaths.length > 5 ? `\n- *…ほか合計 ${allowedPaths.length} 件のワークスペース*` : ''
    return {
      valid: false,
      error: `❌ **登録済みワークスペースにありません**：\`${normalized}\`\n\n**利用可能なワークスペース：**\n${wsDisplay}${more}\n\n> \`/workspaces\` で一覧を確認できます`
    }
  }
  ...
  if (!ok) {
    return {
      valid: false,
      error: `❌ **ワークスペースディレクトリがありません**：\`${normalized}\``
    }
  }
...
async function listSessions(node) {
  ...
  if (workspaces.length > 0) {
    ...
    for (const s of liveList) {
      ...
      if (isSubagentSession(projCache[s.id], s)) continue
      accounted.add(s.id)
      const title = s.title || (s.events ? foldTitle(s.events) : '') || '未分類の会話'
      result.push({
        id: s.id,
        createdAt: s.header?.createdAt || Date.now(),
        cwd: s.header?.cwd || '(未指定)',
        workspaceTitle: 'ワークスペース未指定',
        title,
        events: s.events,
        seq: s.seq ?? 0,
      })
    ...
    for (const s of liveList) {
      accounted.add(s.id)
      const title = s.title || (s.events ? foldTitle(s.events) : '') || 'アクティブな会話'
      result.push({
        id: s.id,
        createdAt: s.header?.createdAt || Date.now(),
        cwd: s.header?.cwd || '(未指定)',
  ...
  switch (command) {
    ...
    case 'resume': {
      ...
      if (!Number.isInteger(index) || index < 1 || index > sessions.length) {
        await node.sendText(`❌ **無効な会話番号**：\`${rest[0] ?? ''}\`\n\n> 使える番号は \`1 – ${sessions.length}\` です（\`/sessions\` で一覧）`)
        return true
      }
      ...
      await node.sendText(`✓ **会話 #${index} に切り替えました**${titleLine}\n- **会話 ID**：\`${fmtSessionId(session.id)}\``)
      return true
    }
    case 'rename': {
      if (!node.activeSessionId) {
        await node.sendText(`❌ **アクティブな会話はありません**\n\n> \`/sessions\` で一覧を見て \`/use 番号\` で切り替えるか、\`/new <プロンプト>\` で作成してください。`)
        return true
      }
      const newTitle = rest.join(' ').trim()
      if (!newTitle) {
        await node.sendText(`❌ **新しいタイトルがありません**\n\n> 使い方：\`/rename <新しいタイトル>\`\n> 例：\`/rename ログイン処理の改善\``)
        return true
      }
      ...
      try {
        ...
        await node.sendText(`✓ **会話名を変更しました**\n- **会話 ID**：\`${fmtSessionId(node.activeSessionId)}\`\n- **新しいタイトル**：${newTitle}`)
      } catch (err) {
        await node.sendText(`❌ **名前の変更に失敗しました**：${err instanceof Error ? err.message : String(err)}`)
      }
      return true
    }
    case 'workspaces': {
      const workspaces = await listWorkspaces(node)
      if (workspaces.length === 0) {
        await node.sendText(`## 🗂️ ワークスペース\n\n> 登録済みのワークスペースはありません。\`/new <プロンプト> @<パス>\` でディレクトリを指定できます。`)
        return true
      }
      ...
      await node.sendText([
        `## 🗂️ ワークスペース（全 ${workspaces.length} 件）`,
        `> 新しい会話：\`/new <プロンプト> @番号\` または \`/new <プロンプト> @パス\``,
        '',
        '| 番号 | ワークスペース名 | パス |',
        '| :--- | :--- | :--- |',
        ...rows,
    ...
    case 'addworkspace': {
      ...
      if (!targetPath) {
        await node.sendText(`❌ **ワークスペースのパスがありません**\n\n> 使い方：\`/addworkspace <絶対パス>\`\n> 例：\`/addworkspace D:\\IdeaProjects\\my-app\``)
        return true
      }
      try {
        const safetyCheck = await isSafeWorkspacePath(targetPath)
        if (!safetyCheck.valid) {
          await node.sendText(`⚠️ **${safetyCheck.error || 'パスの安全チェックに失敗しました'}**：\`${targetPath}\`\n\n> システムの重要ディレクトリや機密設定のパスはワークスペースに登録できません。`)
          return true
        }
        ...
        await node.sendText([
          ...
          `- **名前**：${title}`,
          `- **パス**：\`${resolved}\``,
          foundIndex >= 0 ? `- **ショートカット番号**：\`${numStr}\`` : '',
          '',
          `> \`/new <プロンプト> ${numStr || '@' + resolved}\` でこのワークスペースに会話を作成できます。`,
        ].filter(Boolean).join('\n'))
      } catch (err) {
        await node.sendText(`❌ **ワークスペースの追加に失敗しました**：${err instanceof Error ? err.message : String(err)}`)
      }
      return true
    }
    case 'new': {
      ...
      if (atMatch) {
        ...
        if (/^\d+$/.test(sel)) {
          ...
          if (ws) cwd = ws.path
            ...
            await node.sendText(`❌ **無効なワークスペース番号**：\`${sel}\`\n\n> \`/workspaces\` で一覧と番号を確認してください。`)
            return true
          }
        } else {
    ...
    case 'stop': {
      ...
      if (!agent) {
        await node.sendText(`ℹ️ **実行中の Agent タスクはありません**`)
      } else {
        agent.cancel({ kind: 'user' })
        await node.sendText(`⏹ **現在のタスクの停止を要求しました**`)
      }
      return true
    }
    case 'end': {
      ...
      await node.sendText(`✓ **現在の会話を終了しました**（アクティブな会話はありません）。\n\n> **次の操作**：\n> - \`/new <プロンプト>\` — 新しい会話を開始\n> - \`/sessions\` — 会話一覧\n> - \`/help\` — コマンドヘルプ`)
      return true
    }
    case 'status': {
      const agent = node.activeAgent()
      const session = node.activeSession()
      if (!session) {
        await node.sendText(`## 📊 Agent ステータス\n\n> アクティブな会話はありません。\n> \`/new <プロンプト>\` で開始するか、\`/sessions\` で既存の会話を確認してください。`)
        return true
      }
      const statusMap = {
        idle: '待機 (idle)',
      ...
      const status = statusMap[agent?.status] || (agent?.status ?? '待機 (idle)')
      const lastTurn = [...(session.events ?? [])].reverse().find((e) => e.type === 'turn/end')
      const reason = lastTurn ? describeTurnEnd(lastTurn.data.reason) : 'まだ実行していません'
      const title = session.title || (session.events ? sessionLabel(session) : '')
    ...
    case 'start': // 别名：首次扫码自动开始一个会话
      ...
      return true
    default:
      await node.sendText(`❌ **未知のコマンド**：\`/${command}\`\n\n${helpText()}`)
      return true
  }
}
...
function describeTurnEnd(reason) {
  switch (reason.kind) {
    ...
    case 'aborted': return '⏹ 停止'
    case 'blocked': return '⚠️ ブロック'
    case 'max-tokens': return '⚠️ 出力切り詰め'
...
async function renderSessions(node) {
  ...
  const parts = [
    `## 📋 会話一覧（全 ${all.length} 件）`,
    `> 切り替え：\`/use 番号\` または \`/resume 番号\``,
    '',
  ]
  let idx = 0
  for (const [cwd, group] of groups) {
    const groupName = cwd === '(未指定)' ? '📁 ワークスペース未指定' : `📁 **${group.title || getWorkspaceBasename(cwd)}**`
    parts.push(groupName)
    parts.push('')
    parts.push('| 番号 | タイトル / 要約 | 時刻 | 状態 |')
    parts.push('| :--- | :--- | :--- | :--- |')
    for (const session of group.sessions.slice(0, 20)) {
      idx += 1
      const isActive = session.id === node.activeSessionId
      const statusTag = isActive ? '`[現在]`' : '-'
      const rawTitle = session.title || (session.events ? sessionLabel(session) : '')
      const titleText = formatGoalTitle(rawTitle) || (typeof rawTitle === 'string' ? rawTitle : '') || '新しい会話'
    ...
    if (group.sessions.length > 20) {
      parts.push(`*…このワークスペースは ${group.sessions.length} 件。先頭 20 件のみ表示*`)
    }
    parts.push('')
  }
  if (all.length > 50) parts.push(`*…全 ${all.length} 件。先頭 50 件のみ表示*`)
  return parts.join('\n').trim()
}
...
function helpText() {
  return [
    ...
    '| コマンド | 説明 | 例 |',
    '| :--- | :--- | :--- |',
    '| `/sessions` | 会話一覧（表形式） | `/sessions` または `/list` |',
    '| `/use <番号>` | 指定番号の会話へ切替 | `/use 1` または `/resume 1` |',
    '| `/new <プロンプト>` | 現在のワークスペースで新規会話 | `/new スクリプトを書いて` |',
    '| `/new <語> @N` | 指定ワークスペースで新規会話 | `/new スクリプトを書いて @1` |',
    '| `/rename <新しいタイトル>` | 現在の会話を改名 | `/rename ログイン改善` |',
    '| `/stop` | 実行中のタスクを停止 | `/stop` |',
    '| `/end` | 現在の会話を終了（待機へ） | `/end` |',
    '',
    '### 📁 環境と状態',
    '| コマンド | 説明 |',
    '| :--- | :--- | :--- |',
    '| `/workspaces` | ワークスペース一覧 |',
    '| `/addworkspace <パス>` | 新しいワークスペースを登録 |',
    '| `/status` | Agent の状態ボード |',
    '| `/help` | このヘルプ |',
    '',
    '### 🔐 権限確認',
