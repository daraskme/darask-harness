...
await writeFile(overlay, JSON.stringify([
  { id: 'hmr', disabled: false, config: { base: pathToFileURL(source).href.replace(/\/?$/, '/'), root: [...harness ? ['src', 'packages/dsh-hashline/src', 'packages/dsh-rules/src', 'packages/dsh-status-line/src'] : [], rel('src'), rel('vendor/dsh-bridge-gateway/lib'), rel('vendor/dsh-grok-provider/src')], ignored: ['**/node_modules/**', '**/.*', '**/*.tmp'], debounce: 200 } },
