...
const archives = (await readdir(here)).filter(name => /^dsh-darask-[0-9a-z.+-]+\.tgz$/.test(name));
if (archives.length !== 1) throw new Error('このスクリプトと同じフォルダーに dsh-darask の tgz を一つ置いてください。');
...
let manifest = { name: 'darask-dsh-app', private: true, version: '1.0.0', type: 'module' };
if (existsSync(path.join(data, 'darask/development.json'))) throw new Error('この DSH は Git の開発モードです。DSH の「設定 → 開発・更新」から更新してください。');
try { manifest = JSON.parse(await readFile(path.join(app, 'package.json'), 'utf8')); if (manifest.name !== 'darask-dsh-app') throw new Error('導入先には別のアプリがあります。'); } catch (e) { if (e.code !== 'ENOENT') throw e; }
...
manifest.dependencies = { ...manifest.dependencies, '@deepseek-ai/dsh': '0.1.5-rc.2', 'dsh-darask': `file:../packages/${archiveName}` };
manifest.scripts = { ...manifest.scripts, start: 'node node_modules/dsh-darask/scripts/start.mjs --no-open' };
...
if (profile?.dependencies?.['dsh-darask']) await run(bin, ['plugin', '--profile', 'web', 'remove', 'dsh-darask']);
await run(bin, ['plugin', '--profile', 'web', 'add', archive]);
const launcher = path.join(root, 'start-dsh.mjs');
const entry = path.join(app, 'node_modules/dsh-darask/scripts/start.mjs');
const { writeInstalledLauncher } = await import(pathToFileURL(path.join(app, 'node_modules/dsh-darask/scripts/installed-launcher.mjs')));
...
const { createTailscale } = await import(pathToFileURL(path.join(app, 'node_modules/dsh-darask/src/tailscale.mjs')));
...
if (tail.serve === 'on') {
  ...
  const requirePlugin = createRequire(path.join(app, 'node_modules/dsh-darask/package.json'));
