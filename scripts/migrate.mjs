...
const DEFAULT_SPEC = 'github:daraskme/darask-harness'
...
export async function planMigration(profilePath, spec = DEFAULT_SPEC) {
  ...
  if (typeof spec !== 'string' || !spec || spec !== spec.trim() || spec.length > 2048 || /[\x00-\x1f\x7f]/u.test(spec)) throw new Error('Invalid darask-harness package specification.')
  ...
  const bundles = []
  const hasDarask = manifest.dsh.profile.bundles.includes('darask-harness')
  let added = false
  for (const name of manifest.dsh.profile.bundles) {
    if (LEGACY.has(name)) {
      if (!hasDarask && !added) { bundles.push('darask-harness'); added = true }
    } else if (name !== 'darask-harness' || !added) {
      bundles.push(name)
      if (name === 'darask-harness') added = true
    }
  }
  if (!added) bundles.push('darask-harness')
  ...
  next.dependencies['darask-harness'] = spec
  ...
  return {
    ...
    summary: {
      ...
      dependency: { name: 'darask-harness', spec }, patchReviewRequired: patchExists,
