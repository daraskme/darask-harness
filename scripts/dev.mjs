...
const linkedName = 'darask-harness';
if (!source || source !== packageRoot) throw new Error('Git で管理している darask-harness のチェックアウトから起動してください。');
...
await writeFile(overlay, JSON.stringify([
  { id: 'hmr', disabled: false, config: { base: pathToFileURL(source).href.replace(/\/?$/, '/'), root: ['src', 'packages/dsh-hashline/src', 'packages/dsh-rules/src', 'packages/dsh-status-line/src', 'vendor/dsh-bridge-gateway/lib', 'vendor/dsh-grok-provider/src'], ignored: ['**/node_modules/**', '**/.*', '**/*.tmp'], debounce: 200 } },
