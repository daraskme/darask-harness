...
function closeOutputItem(
  ...
  if (block.type === "reasoning") {
    ...
    if (event.item.encrypted_content !== undefined) {
      ...
      // An encrypted-only reasoning item is still a replay block. Keep an
      // empty native reasoning block so content and replay positions stay aligned.
      if (!block.visible && !block.reusedReasoning && completedServerCalls.length === 0) {
        revealReasoningBlock(block, index, unresolvedReasoningIndexes, replayBlocks, chunks)
