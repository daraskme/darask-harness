...
import { createDshSession, upstreamHeaders } from './dsh-session.mjs';
import { attachOAuthOrigin } from './oauth-origin.mjs';
...
function listAllLanIPv4() {
  ...
  for (const [ifname, addrs] of Object.entries(interfaces)) {
    ...
    for (const addr of addrs) {
      ...
      let label = ifname;
      if (lower.includes('wi-fi') || lower.includes('wlan') || lower.includes('wireless')) label += ' (Wi-Fi)';
      else if (lower.includes('ethernet') || lower.includes('以太网') || lower.includes('eth') || lower.includes('en')) label += ' (有線LAN)';
      else if (isVirtual) label += ' (仮想NIC / WSL / VM)';
...
class ProxyServer {
   ...
   */
  constructor({ localPort, targetPort, authManager, logger, tls, tag = '', extraTargets = [], dshSession = () => '' }) {
  ...
  async start() {
    ...
    const handler = async (req, res) => {
        ...
        req.on('end', () => {
          try {
            ...
            if (verify?.success) {
              ...
              res.end(JSON.stringify({ ok: false, error: verify?.error || 'アクセスパスワードが違います' }));
            }
          } catch (e) {
            res.writeHead(400, { 'Content-Type': 'application/json; charset=utf-8' });
            res.end(JSON.stringify({ ok: false, error: '無効なリクエストです' }));
          }
        });
        return;
      }
      ...
      if (pathname === '/__dsh_bridge__/loopback-token') {
        ...
        res.end(JSON.stringify({ ok: false, error: '本機からのみアクセスできます' }));
        return;
      }

      // 4. 核心鉴权拦截
      let auth;
      try { auth = this.authManager ? await this.authManager.verifyRequestAsync(req) : { authenticated: true }; }
      catch { auth = { authenticated: false }; }
      if (auth.cfAccessRequired) {
        const accept = String(req.headers['accept'] || '');
        const isHtml = accept.includes('text/html') || (!req.url.startsWith('/api/') && !req.url.includes('.'));
        if (isHtml) {
          res.writeHead(401, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' });
          res.end('<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Cloudflare Access - DeepSeek Harness</title></head><body style="font-family:system-ui,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#0f172a;color:#e2e8f0"><div style="text-align:center;max-width:420px;padding:24px"><h1 style="font-size:20px">Cloudflare Zero Trust 認証が必要です</h1><p style="font-size:14px;line-height:1.7;opacity:.85">このサイトは Cloudflare Access で保護されています。公開 URL からアクセスすると、Cloudflare のログイン画面に転送されます。ログイン後にもこの画面が出る場合は、Gateway の Cloudflare Access 設定（チームドメイン・AUD）を確認してください。</p></div></body></html>');
        } else {
          res.writeHead(401, { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' });
          res.end(JSON.stringify({ error: 'unauthorized', message: 'Cloudflare Access の認証が必要です' }));
        }
        return;
      }
      ...
      if (!auth.authenticated) {
        ...
        if (isHtml) {
          ...
          res.writeHead(401, { 'Content-Type': 'application/json; charset=utf-8' });
          res.end(JSON.stringify({ error: 'unauthorized', message: 'アクセス認証が必要です。先にログインしてください' }));
          return;
        }
      }
      ...
      if (this.extraTargets && this.extraTargets.length) {
        ...
        if (extra) {
          ...
          const upstreamPath = extra.stripPrefix
            ...
            : req.url;
          const headers = upstreamHeaders(req.headers, extra.targetPort);
      ...
      // 5. 认证通过：正常执行反向代理转发
      const headers = upstreamHeaders(req.headers, this.targetPort, this.dshSession());
      try { attachOAuthOrigin(headers, req, !!this.tls); }
      catch { res.writeHead(403, { 'Cache-Control': 'no-store' }); res.end('接続元を確認できません。'); return; }
      const proxyReq = httpRequest(
        { host: '127.0.0.1', port: this.targetPort, method: req.method, path: req.url, headers, agent: false },
        (proxyRes) => {
          const contentType = String(proxyRes.headers['content-type'] ?? '');
          // 未压缩的 HTML ドキュメント注入 polyfill
 ...
 */
class BridgeService {
  constructor({ dshPort, proxyPort, home, cloudflaredConfig, customTunnelConfig, lanConfig, gatewayConfig, authManager, onPersist, logger, connection }) {
    this.dshPort = dshPort;
    this.dshSession = createDshSession(connection, dshPort);
    this.proxyPort = proxyPort;
    this.home = home;
    this.cloudflaredConfig = cloudflaredConfig ?? { token: '', hostname: '', autoStart: false };
    this.customTunnelConfig = null;
  ...
  async startProxy() {
    if (this._disposed) throw new Error('Gateway は終了しています');
    if (this.proxy) return this.proxy;
    // Plugin startup and tunnel auto-start may enter before port discovery
    // completes. Share the whole operation, including the listen result.
    if (this._proxyStarting) return this._proxyStarting;
    ...
    this._proxyStarting = starting;
    try { return await starting; }
    finally { if (this._proxyStarting === starting) this._proxyStarting = null; }
  }
  ...
  async startCustomTunnel({ autoStart = true } = {}) {
    throw new Error('この機能は DARASK から削除されました。');
    if (this.customTunnel) {
      throw new Error('自前トンネルは既に起動しています');
    }
    ...
    if (!serverUrl || !accessToken) {
      throw new Error('自前トンネルの接続先とアクセストークンを設定してください');
    }
  ...
  async startCloudflared({ autoStart = true } = {}) {
    if (this.cloudflared) {
      throw new Error('Cloudflare トンネルは既に起動しています');
    }
    ...
    if (this._disposed) throw new Error('Gateway は終了しています');
    this.cloudflaredState = { phase: 'connecting', detail: '初期化しています…' };
    ...
    // 非阻塞启动，立即返回——ダウンロード/连接进度通过 onStateChange 推送
    this.cloudflared.start();
  }

  async stopCloudflared({ preserveAutoStart = false } = {}) {
    if (this.cloudflared) {
      this.cloudflared.stop();
      this.cloudflared = null;
      this.cloudflaredState = { phase: 'idle', detail: '' };
    }
    // Unloading for HMR/shutdown is not the user's request to disable startup.
    if (preserveAutoStart) return;
    this.cloudflaredConfig = {
      ...(this.cloudflaredConfig ?? {}),
      autoStart: false,
    };
  ...
  // 重置 Cloudflare 隧道：关闭隧道 + 删除已ダウンロード的 cloudflared 二进制
  ...
  async startGateway() {
    ...
    try {
      ...
      throw new Error(`HTTPS の自己署名証明書を生成できません: ${err?.message ?? err}`);
    }
  ...
  async listRemoteDirectories(targetPath) {
    ...
    // 1. 获取快速访问常用ルートフォルダ
    const roots = [
      { name: '🏠 ホームフォルダ', path: home },
    ];
    const commonSubdirs = [
      { name: '💻 デスクトップ', sub: 'Desktop' },
      { name: '📁 ドキュメント', sub: 'Documents' },
      { name: '📥 ダウンロード', sub: 'Downloads' },
    ...
    if (isWin) {
      ...
      for (const letter of letters) {
        ...
        try {
          ...
          drives.push({ name: `${letter}: ドライブ`, path: driveRoot });
        } catch {}
      }
      if (drives.length === 0) drives.push({ name: 'C: ドライブ', path: 'C:\\' });
    } else {
      drives.push({ name: 'ルートフォルダ /', path: '/' });
    }
    ...
    if (isWin) {
      ...
      breadcrumbs.push({ name: 'ルートフォルダ /', path: '/' });
  ...
  async diagnoseNetwork() {
    ...
    results.push({
      item: 'local_proxy',
      name: `本機のプロキシポート (${this.proxyPort})`,
      status: this.proxy ? 'pass' : 'fail',
      detail: this.proxy ? `稼働中（転送先ポート: ${this.dshPort})` : 'プロキシは停止中',
    });
    ...
    results.push({
      item: 'lan_interface',
      name: 'LAN の IP アドレスと接続状態',
      status: lanIp ? 'pass' : 'warn',
      detail: lanIp ? `有効な LAN IPv4: ${lanIp}` : '有効な LAN IPv4 がありません。Wi-Fi または有線 LAN を確認してください',
    });
    ...
    try {
      ...
      results.push({
        ...
        name: 'Cloudflare エッジネットワーク',
        status: 'pass',
        latencyMs: cfLatency,
        detail: `接続成功（遅延 ${cfLatency}ms)`,
      });
    } catch (err) {
      results.push({
        item: 'cloudflare_edge',
        name: 'Cloudflare エッジネットワーク',
        status: 'warn',
        detail: `接続エラー: ${err.message} （一時トンネルが制限されている可能性があります）`,
      });
    }
    ...
    try {
      ...
      results.push({
        item: 'npmmirror',
        name: 'npm ミラー（npmmirror）',
        status: 'pass',
        latencyMs: npmLatency,
        detail: `接続成功（遅延 ${npmLatency}ms)`,
      });
    } catch (err) {
      results.push({
        item: 'npmmirror',
        name: 'npm ミラー（npmmirror）',
        status: 'warn',
        detail: `接続タイムアウトまたはエラー: ${err.message}`,
      });
    }
    ...
    if (customServerUrl) {
      ...
      try {
        ...
        results.push({
          ...
          name: `自前トンネルサーバー (${parsedUrl.hostname}${parsedUrl.port ? `:${parsedUrl.port}` : ''})`,
          status: 'pass',
          latencyMs: ctLatency,
          detail: `サーバー接続成功（遅延 ${ctLatency}ms · 状態: ${isRunning ? 'クライアント接続中' : '接続待ち'})`,
        });
      } catch (err) {
        if (isRunning) {
          results.push({
            item: 'custom_tunnel_server',
            name: `自前トンネルサーバー (${customServerUrl})`,
            status: 'pass',
            detail: 'クライアント接続中 (WebSocket 接続済み)',
          });
        } else {
          results.push({
            item: 'custom_tunnel_server',
            name: `自前トンネルサーバー (${customServerUrl})`,
            status: 'warn',
            detail: `自前サーバーに接続できません: ${err.message}`,
          });
        }
      }
    } else {
      results.push({
        ...
        name: '自前トンネルサーバー',
        status: 'pass',
        detail: '自前サーバーは未設定です。「外部アクセス」で設定できます',
      });
    }
  ...
  async dispose() {
    this._disposed = true;
    await this._proxyStarting?.catch(() => {});
    this.stopCustomTunnel();
    await this.stopCloudflared({ preserveAutoStart: true });
    if (this.proxy) {
...
async function apply(ctx, config = {}) {
  ...
  async function checkEmergencyReset() {
    try {
      ...
      authManager.cfAccess = { enabled: false, teamDomain: '', aud: '' };
  ...
  await checkEmergencyReset().then(() => loadConfig()).then(async (stored) => {
    if (stored?.auth) {
      ...
      if (stored.auth.cfAccess) await authManager.setCfAccess(stored.auth.cfAccess, { persist: false });
      logger.info('dsh-bridge: loaded saved auth config (enabled=%s, mode=%s, adminPolicy=%s, cfAccess=%s)', authManager.enabled, authManager.mode, authManager.adminPolicy, authManager.cfAccessConfigured ? 'on' : 'off');
    }
  });
  ...
  const disposeRpc = installBridgeRpc(ctx, {
    ...
    importBackup: async (backup) => {
      if (!backup || typeof backup !== 'object' || !backup.config || typeof backup.config !== 'object') {
        throw new Error('バックアップの形式が無効です。config がありません');
      }
      ...
      if (incoming.auth) {
        ...
        if (incoming.auth.cfAccess) await authManager.setCfAccess(incoming.auth.cfAccess, { persist: false });
      }
      // 重新载入 Tunnels
