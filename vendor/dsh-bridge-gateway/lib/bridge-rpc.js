...
import { removedFeatureRequest } from './darask-features.mjs';
...
function checkAdminAuth(authManager, payload) {
  ...
  return fail('bad-request', '操作は拒否されました。管理者権限が必要です。管理パスワードでロックを解除してください');
}
...
export function installBridgeRpc(ctx, { service, authManager, wechat, platformManager, logger, saveCustomTunnelConfig, exportBackup, importBackup }) {
  ...
  const handleBridgeRpc = async (endpoint, payload = {}, signal) => {
      if (removedFeatureRequest(endpoint, payload)) return fail('bad-request', 'この機能は DARASK から削除されました。');
      ...
      try {
        ...
        if (endpoint === BRIDGE_ENDPOINTS.authUpdateConfig) {
          ...
          const { enabled, mode, scope, adminPolicy, password, adminPassword, cfAccess } = payload;
          ...
          if (cfAccess !== undefined) {
            if (cfAccess === null || typeof cfAccess !== 'object') return fail('bad-request', 'Cloudflare Access 設定が無効です');
            await authManager.setCfAccess(cfAccess);
          }
