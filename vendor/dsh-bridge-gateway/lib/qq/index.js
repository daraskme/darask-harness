...
export class QqService extends Platform {
  ...
  async login(opts = {}) {
    ...
    if (!this.gateway.configured) {
      return { ok: false, error: '先に設定パネルで QQ 開放プラットフォームの AppID と ClientSecret を入力してください' }
    }
  ...
  async _ensurePanelSetup() {
    ...
    const items = [
      { name: '/new', desc: '新しい会話', type: 'command' },
      { name: '/list', desc: '会話一覧', type: 'command' },
      { name: '/resume', desc: '会話を再開', type: 'command' },
      { name: '/sessions', desc: '会話を切替', type: 'command' },
      { name: '/help', desc: 'コマンドヘルプ', type: 'command' },
    ]
    ...
    try {
      const menuItems = [
        { name: '新規', type: 'send_message', send_message: '/new' },
        { name: '一覧', type: 'send_message', send_message: '/list' },
        { name: 'ヘルプ', type: 'send_message', send_message: '/help' },
      ]
...
const PANEL_REMARK = 'dsh-bridge よく使うコマンド'
