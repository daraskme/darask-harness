...
import { t, installLocale } from './i18n.js';
...
const GITHUB_URL = 'https://github.com/daraskme/dsh-bridge-gateway';
const RELEASES_URL = 'https://github.com/daraskme/dsh-bridge-gateway/releases';
const ISSUES_URL = 'https://github.com/daraskme/dsh-bridge-gateway/issues/new';
const TUNNEL_DOCS_URL = 'https://github.com/daraskme/dsh-bridge-gateway/blob/main/docs/custom-tunnel.ja.md';
...
function isAdminBlockedMessage(msg) {
  const s = String(msg || '');
  return /管理员权限|管理密码解锁|管理者権限|管理パスワード|admin (password|unlock|privile)/i.test(s);
}
  ...
  return React.createElement('div', { style: { marginTop: 10 } },
    auth?.enabled
      ? React.createElement('div', {
          ...
          title: onNavigateSecurity ? t('qr.authOnTitle') : undefined,
        },
      ...
      : React.createElement('div', {
          ...
          onClick: onNavigateSecurity,
          title: onNavigateSecurity ? t('qr.authOffTitle') : undefined,
        },
  ...
  return React.createElement('div', {
    ...
    React.createElement('select', {
      ...
      interfaces.map((iface) => React.createElement('option', {
        ...
        value: iface.address,
      }, `${iface.address} · ${iface.label || iface.name}${iface.isVirtual ? t('lan.virtualTag') : ''}`)),
    ),
  );
});
...
const AccessAuthCard = React.memo(function AccessAuthCard({ auth, rpcCall, onUpdate }) {
  ...
  // Cloudflare Zero Trust (Access) 状态
  ...
  return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 14 } },
    ...
    enabled && React.createElement(React.Fragment, null,
      ...
      React.createElement('div', { style: s.card },
        ...
        React.createElement('div', { style: { marginBottom: 16 } },
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap', marginBottom: 8 } },
...
function PlatformCard({ platformId, platformName, platformDesc, rpcCall, onStatusChange }) {
  ...
  const statusLabel = platform?.status === 'connected' ? t('status.connected')
    : platform?.status === 'starting' ? t('status.connecting')
    : platform?.status === 'reconnecting' ? t('status.reconnecting')
    : platform?.status === 'paused' ? t('status.pausedExpired')
    : platform?.status === 'error' ? t('status.errorShort')
    ...
    platform?.configured && React.createElement('div', { style: s.block },
          ...
          ? platform.allowFrom.map((id) =>
              React.createElement('span', { key: id, style: { ...s.tag, background: 'var(--dsw-alias-bg-layer-2,#f3f4f6)', color: 'var(--dsw-alias-label-primary,currentColor)', gap: 6 } },
                ...
                React.createElement('button', {
                  ...
                  onClick: () => removeAllow(id), title: t('im.removeAllow'),
                }, '×'),
              )
            )
          : React.createElement('div', { style: { ...s.muted, fontSize: 12 } },
              platformId === 'wechat'
                ? t('im.allowEmptyWechat')
                : t('im.allowEmpty')
            )),
      ),
      ...
      React.createElement('div', { style: { display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap', alignItems: 'center' } },
        platform.status !== 'connected' && platform.status !== 'starting' &&
          React.createElement('button', { style: s.btnPri, onClick: onLogin, disabled: busy }, t('btn.reconnect')),
        (platform.status === 'connected' || platform.status === 'starting') &&
          React.createElement('button', { style: s.btnGhost, onClick: onStop, disabled: busy }, t('btn.disconnect')),
        React.createElement('button', {
          ...
          disabled: busy,
          onClick: () => { if (window.confirm(t('im.unbindConfirm'))) act(BRIDGE_ENDPOINTS.platformUnbind, {}); },
      ...
      showQr && login.qr
        ? React.createElement('div', null,
            ...
            React.createElement('div', { style: { ...s.muted, marginTop: 4 } },
              login.phase === 'scaned'
                ? t('im.scanned')
                : (platformId === 'wechat' ? t('im.scanWechat') : t('im.scanGeneric'))
            ),
...
function RemoteWorkspaceCard({ rpcCall }) {
  ...
  return React.createElement('div', { style: { ...s.card, marginBottom: 16 } },
    ...
    }, loading ? t('ops.wsLoading') : t('ops.wsEmpty')),
  );
}
...
function VersionBanner({ rpcCall }) {
  ...
  const handleUpgrade = React.useCallback(async () => {
    ...
    try {
      ...
      if (r?.ok && r.value?.ok) {
        setUpgradeResult({ ok: true, message: t('ver.upgraded', { latest: info.latest }) });
      } else {
        setUpgradeResult({ ok: false, message: r?.value?.error || r?.error?.message || t('ver.upgradeFail') });
        setShowManual(true);
      }
    } catch (e) {
      setUpgradeResult({ ok: false, message: e.message || t('ver.upgradeReqFail') });
      setShowManual(true);
    } finally {
      setUpgrading(false);
    }
  ...
  const handleRestart = React.useCallback(async () => {
    setRestarting(true);
    setRestartStatus({ phase: 'restarting', text: t('ver.restartScheduling') });
    try {
      await rpcCall(BRIDGE_ENDPOINTS.restartDsh, {});
    } catch {
    ...
    setRestartStatus({ phase: 'reconnecting', text: t('ver.restartReconnecting') });
    ...
    const pollHealth = setInterval(async () => {
      ...
      try {
        ...
        if (r?.ok) {
          clearInterval(pollHealth);
          setRestartStatus({ phase: 'success', text: t('ver.restartLoaded') });
          setTimeout(() => {
      ...
      if (attempts >= maxAttempts) {
        clearInterval(pollHealth);
        setRestartStatus({ phase: 'timeout', text: t('ver.restartTimeout') });
        setRestarting(false);
      }
    }, 1000);
  }, [rpcCall]);
        ...
        React.createElement('span', {
          style: {
            ...
            background: hasUpdate
              ? 'var(--dsw-alias-state-info-bg, var(--dsw-alias-bg-layer-2, #eff6ff))'
              : isLatest
                ? 'var(--dsw-alias-state-success-bg, var(--dsw-alias-bg-layer-2, #ecfdf5))'
          ...
          loading
            ...
            : isLatest
              ...
              : null,
          info ? `v${info.current}` : t('ver.checking'),
          isLatest && React.createElement('span', { style: { opacity: 0.85, fontSize: 11, fontWeight: 400 } }, t('ver.latest')),
          hasUpdate && React.createElement('span', { style: { fontWeight: 600, fontSize: 11 } }, `➔ v${info.latest}`),
          info?.error && React.createElement('span', { style: { color: 'var(--dsw-alias-state-warn-primary,#d97706)', fontSize: 11 } }, t('ver.timeout')),
        ),
        // 刷新检查按钮
        React.createElement('button', {
          style: {
          ...
          React.createElement(Icons.refresh),
          loading ? t('ver.checkBusy') : t('ver.check'),
        ),
      ),
      links,
    ),
      ...
      React.createElement('div', { style: { display: 'flex', alignItems: 'flex-start', gap: 12 } },
        ...
        React.createElement('div', { style: { flex: 1, minWidth: 0 } },
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 8 } },
            ...
            }, t('ver.newVersion', { latest: info.latest, current: info.current })),
            React.createElement('button', {
              style: {
                ...s.btnPri,
                height: 28,
              ...
              upgrading
                ...
                : upgradeResult?.ok
                  ? t('ver.upgradeDone')
                  : t('ver.upgradeTo', { latest: info.latest })
            ),
          ),
          ...
          info?.releaseNotes && React.createElement('div', {
            ...
            React.createElement('div', { style: { fontWeight: 600, color: 'var(--dsw-alias-state-info-primary, #2563eb)', marginBottom: 2 } }, t('ver.highlights')),
            info.releaseNotes
          ),
          ...
          upgradeResult?.ok && !dismissRestart && React.createElement('div', {
            ...
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 } },
              ...
              }, t('ver.needRestart', { latest: info.latest })),
            ),
            ...
            (restarting || restartStatus) && React.createElement('div', {
              ...
              restartStatus?.text || t('ver.processing'),
            ),
          ),
          ...
          React.createElement('div', { style: { marginTop: 4 } },
            ...
            }, showManual ? t('ver.collapseManual') : t('ver.showManual')),
          ),
...
function BridgePanel({ rpcCall }) {
  ...
  const handleUnlockAdmin = React.useCallback(async (e) => {
    ...
    try {
      ...
      setUnlockErr(err.message || t('err.unlockFail'));
    } finally {
      setUnlocking(false);
    }
  ...
  return React.createElement('div', { style: { maxWidth: 620, position: 'relative' } },
    ...
    !isLocalhost && adminUnlocked && React.createElement('div', {
      style: {
        ...
        padding: '8px 14px', background: 'var(--dsw-alias-state-info-bg, var(--dsw-alias-bg-layer-2, #eff6ff))',
    ...
    !isLocalhost && !adminUnlocked && auth?.enabled && policy !== 'open' && React.createElement('div', {
      style: {
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        padding: '8px 14px', background: 'var(--dsw-alias-state-warn-bg, var(--dsw-alias-bg-layer-2, #fffbeb))',
...
function setupMobileExperience(rpcCall, ctx) {
  ...
  if (!header) {
    ...
    leftBtn.title = t('mobile.openMenu');
    leftBtn.innerHTML = `
    ...
    leftBtn.onclick = (e) => {
      ...
      if (isOpen) {
        const expand = document.querySelector('button[aria-label*="打开侧边栏"], button[title*="打开侧边栏"], button[aria-label*="サイドバーを開く"], button[title*="サイドバーを開く"], button[aria-label*="Open sidebar"], button[title*="Open sidebar"]');
    ...
    titleEl.innerText = t('mobile.newSessionFallback');
    titleEl.onclick = () => {
    ...
    rightBtn.title = t('mobile.newSession');
    rightBtn.innerHTML = `
    ...
    rightBtn.onclick = () => {
      const openPanels = document.querySelectorAll('div[class*="nArs4W_panel"]:not([class*="panelHidden"]), div[class*="workbench_panel"]:not([class*="panelHidden"])');
      openPanels.forEach((p) => p.classList.add('nArs4W_panelHidden'));
      const dshNewBtn = document.querySelector('button[aria-label="新建会话"], button[aria-label="新しい会話"], button[aria-label="New conversation"]');
  ...
  const syncMobileTitle = () => {
    ...
    if (!cur || cur.blank) {
      ...
      titleEl.innerText = cur.displayTitle || cur.title || t('mobile.sessionFallback');
    }
  };

  const refreshMobileChromeStrings = () => {
    const menuBtn = document.querySelector('.dsh-header-menu-btn');
    if (menuBtn) menuBtn.title = t('mobile.openMenu');
    const newBtn = document.querySelector('.dsh-header-new-btn');
    if (newBtn) newBtn.title = t('mobile.newSession');
    document.querySelectorAll('.dsh-mobile-panel-close-btn span').forEach((el) => {
      el.textContent = t('mobile.backToChat');
    });
    syncMobileTitle();
  };
  ...
  if (typeof ctx?.locale?.subscribe === 'function') {
    ctx.locale.subscribe(refreshMobileChromeStrings);
  }
  ...
  const ensurePanelCloseButton = () => {
    ...
    panels.forEach((p) => {
      ...
      if (bar && !bar.querySelector('.dsh-mobile-panel-close-btn')) {
        ...
        btn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg><span>${t('mobile.backToChat')}</span>`;
        btn.onclick = (e) => {
          e.stopPropagation();
          document.body.classList.remove('dsh-workbench-open');
          p.classList.add('nArs4W_panelHidden');
          const collapseBtn = document.querySelector('button[class*="toggleButton"][aria-label*="收起"], button[class*="toggleButton"][aria-label*="折りたたむ"], button[class*="toggleButton"][aria-label*="Collapse"]');
  ...
  const ensureTabButton = () => {
    ...
    tabBtn.innerHTML = `<span class="dsh-mobile-tab-btn-label">${t('mobile.chatFallback')}</span><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>`;
    tabBtn.onclick = (e) => {
      e.stopPropagation();
  ...
  document.addEventListener('click', (e) => {
    if (typeof window === 'undefined' || window.innerWidth > 768) return;
    const trigger = e.target.closest('button[aria-label*="面板"], button[aria-label*="工作区"], button[aria-label*="パネル"], button[aria-label*="ワークスペース"], button[aria-label*="Panel"], button[aria-label*="Workspace"], div[class*="toggleCluster"] button, button[class*="subagent"], div[class*="headerActions"] button, div[class*="titleRow"] button');
  ...
  document.addEventListener('click', (e) => {
    if (typeof window === 'undefined' || window.innerWidth > 768) return;
    const toggle = e.target.closest('button[aria-label*="收起侧边栏"], button[title*="收起侧边栏"], button[aria-label*="サイドバーを閉じる"], button[title*="サイドバーを閉じる"], button[aria-label*="Collapse sidebar"], button[title*="Collapse sidebar"], button[aria-label*="Close sidebar"], button[title*="Close sidebar"]');
    if (toggle) {
      document.body.classList.remove('dsh-drawer-open');
    }
  }, true);
  ...
  document.addEventListener('click', (e) => {
    ...
    if (sidebar && sidebar.contains(e.target)) {
      ...
      if (btn) {
        ...
        if (label.includes('新会话') || label.includes('新しい会話') || label.includes('New conversation') || btn.matches('button[class*="newSession"]')) {
          document.body.classList.remove('dsh-drawer-open');
          return;
        }
      }
  ...
  window.addEventListener('touchstart', (e) => {
    ...
    longPressTimer = setTimeout(() => {
      ...
      // 寻找该会话项内的三点操作按钮并触发点击
      const actionBtn = sessionRow.querySelector('button[aria-label*="操作"], button[aria-label*="Actions"], button[class*="iconButton"], button');
      if (actionBtn) {
        actionBtn.click();
      }
    }, 380);
  ...
  document.addEventListener('click', (e) => {
    ...
    const isAddWorkspace = (
      ...
      label.includes('ワークスペースを追加') ||
      label.includes('ワークスペースを開く') ||
      label.includes('フォルダーを開く') ||
      label.includes('Add workspace') ||
      label.includes('Open workspace') ||
      label.includes('Open folder') ||
      btn.matches('button[aria-label*="工作区"][aria-label*="添加"], button[aria-label*="工作区"][aria-label*="打开"], button[aria-label*="ワークスペース"][aria-label*="追加"], button[aria-label*="ワークスペース"][aria-label*="開く"], button[aria-label*="Workspace"][aria-label*="Add"], button[aria-label*="Workspace"][aria-label*="Open"]')
    );
...
function showRemoteWorkspaceDialog(rpcCall, onWorkspaceAdded, clientCtx, onPicked, onCancel) {
  ...
  function render() {
    ...
    modal.innerHTML = `
      ...
      <div style="padding: 12px 16px; border-bottom: 1px solid var(--dsw-alias-border-l2, #e5e7eb); display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; background: var(--dsw-alias-bg-layer-2, #f9fafb);">
        <div style="display: flex; align-items: center; gap: 8px; overflow: hidden;">
          ...
          <div style="overflow: hidden;">
            <div style="font-size: 15px; font-weight: 600; color: var(--dsw-alias-label-primary, #111827); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${t('ws.pickTitle')}</div>
            <div style="font-size: 11px; color: var(--dsw-alias-label-tertiary, #6b7280); margin-top: 1px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${t('ws.pickSubtitle')}</div>
          </div>
        </div>
      ...
      <div style="padding: 12px 16px; overflow-y: auto; flex: 1; display: flex; flex-direction: column; gap: 10px;">
        <!-- 提示信息横幅 -->
        ${statusMessage ? `
          <div style="padding: 8px 12px; border-radius: 8px; font-size: 12px; line-height: 1.5; font-weight: 500; display: flex; align-items: center; gap: 8px; ${isErrorMessage ? 'background: var(--dsw-alias-state-error-bg, var(--dsw-alias-bg-layer-2, #fef2f2)); border: 1px solid var(--dsw-alias-state-error-border, #fecaca); color: var(--dsw-alias-state-error-primary, #dc2626);' : 'background: var(--dsw-alias-state-success-bg, var(--dsw-alias-bg-layer-2, #ecfdf5)); border: 1px solid var(--dsw-alias-state-success-border, #a7f3d0); color: var(--dsw-alias-state-success-primary, #059669);'}">
        ...
        <div style="background: var(--dsw-alias-bg-layer-3, #f3f4f6); border: 1px solid var(--dsw-alias-border-l2, #e5e7eb); border-radius: 10px; padding: 6px 10px; display: flex; align-items: center; justify-content: space-between; gap: 6px;">
          ...
          <div style="display: flex; align-items: center; gap: 4px; flex-shrink: 0;">
            ${parentPath ? `
              <button id="dsh-ws-up-btn" data-path="${escapeHtml(parentPath)}" title="${t('ws.upTitle')}" style="border: 1px solid var(--dsw-alias-border-l2, #d1d5db); background: var(--dsw-alias-bg-layer-1, #fff); color: var(--dsw-alias-label-primary, #111827); padding: 3px 8px; border-radius: 6px; font-size: 11px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 2px;">
                ${t('ws.up')}
              </button>
            ` : ''}
            <button id="dsh-ws-refresh-btn" title="${t('ws.refresh')}" style="border: 1px solid var(--dsw-alias-border-l2, #d1d5db); background: var(--dsw-alias-bg-layer-1, #fff); color: var(--dsw-alias-label-primary, #111827); padding: 3px 6px; border-radius: 6px; font-size: 11px; cursor: pointer;">
              🔄
            </button>
          </div>
        </div>
        ...
        <div style="background: var(--dsw-alias-state-info-bg, var(--dsw-alias-bg-layer-2, #eff6ff)); border: 1px solid var(--dsw-alias-state-info-border, #bfdbfe); border-radius: 10px; padding: 10px 12px; display: flex; flex-direction: column; gap: 6px;">
          <div style="display: flex; align-items: center; justify-content: space-between; gap: 6px;">
            <span style="font-size: 11px; font-weight: 600; color: var(--dsw-alias-brand-primary, #2563eb); flex-shrink: 0;">${t('ws.currentDir')}</span>
        ...
        <div style="border: 1px solid var(--dsw-alias-border-l2, #e5e7eb); border-radius: 10px; overflow: hidden; display: flex; flex-direction: column; background: var(--dsw-alias-bg-layer-1, #fff);">
          ...
          <div style="padding: 7px 10px; background: var(--dsw-alias-bg-layer-2, #f9fafb); border-bottom: 1px solid var(--dsw-alias-border-l2, #e5e7eb); display: flex; align-items: center; justify-content: space-between; gap: 6px;">
            <div style="display: flex; align-items: center; gap: 6px; flex: 1;">
              ...
              <input id="dsh-ws-filter-input" type="text" value="${escapeHtml(filterQuery)}" placeholder="${t('ws.filterPlaceholder')}" style="border: none; background: transparent; font-size: 12px; width: 100%; color: var(--dsw-alias-label-primary, #111827); outline: none;" />
            </div>
            <span style="font-size: 10px; color: var(--dsw-alias-label-tertiary, #6b7280); flex-shrink: 0;">
              ${t('ws.folderCount', { n: filteredEntries.length })}
            </span>
          </div>
          ...
          <div style="max-height: 240px; min-height: 120px; overflow-y: auto; padding: 2px 0;">
            ...
            ` : filteredEntries.length === 0 ? `
              <div style="padding: 26px 16px; text-align: center; font-size: 12px; color: var(--dsw-alias-label-tertiary, #6b7280); display: flex; flex-direction: column; align-items: center; gap: 4px;">
                ...
                <span>${filterQuery ? t('ws.noMatch') : t('ws.noChildren')}</span>
                <span style="font-size: 11px; color: var(--dsw-alias-label-tertiary, #9ca3af);">${t('ws.clickBlue')}</span>
              </div>
            ` : filteredEntries.map(e => `
              <div class="dsh-ws-entry-row" data-path="${escapeHtml(e.path)}" style="display: flex; align-items: center; justify-content: space-between; padding: 9px 12px; border-bottom: 1px solid var(--dsw-alias-border-l2, #f3f4f6); cursor: pointer; font-size: 12px; transition: background 0.1s; min-height: 40px;">
                ...
                <button class="dsh-ws-pick-entry-btn" data-path="${escapeHtml(e.path)}" title="${t('ws.pickEntryTitle')}" style="border: 1px solid var(--dsw-alias-state-success-border, #a7f3d0); background: var(--dsw-alias-state-success-bg, var(--dsw-alias-bg-layer-2, #ecfdf5)); color: var(--dsw-alias-state-success-primary, #059669); padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 2px; flex-shrink: 0; margin-left: 8px; white-space: nowrap; transition: all 0.1s;">
                  ${t('ws.pickEntry')}
                </button>
              </div>
            `).join('')}
        ...
        <div>
          <div style="display: flex; align-items: center; justify-content: space-between;">
            <button id="dsh-ws-toggle-manual" style="border: none; background: none; color: var(--dsw-alias-label-tertiary, #6b7280); font-size: 11px; cursor: pointer; padding: 2px 0; text-decoration: underline;">
              ${showManualInput ? t('ws.hideManual') : t('ws.showManual')}
            </button>
          </div>
          ${showManualInput ? `
            <div style="margin-top: 6px; display: flex; gap: 6px;">
              <input id="dsh-ws-manual-input" type="text" value="${escapeHtml(currentPath)}" placeholder="${t('ws.pathPlaceholder')}" style="flex: 1; font-family: ui-monospace, Menlo, monospace; font-size: 11px; padding: 6px 8px; border-radius: 8px; border: 1px solid var(--dsw-alias-border-l2, #d1d5db); background: var(--dsw-alias-bg-layer-1, #fff); color: var(--dsw-alias-label-primary, #111827); outline: none;" />
              ...
              <button id="dsh-ws-manual-add-btn" style="border: none; background: var(--dsw-alias-brand-primary, #4f6ef7); color: var(--dsw-alias-label-primary-foreground, #fff); padding: 0 12px; border-radius: 8px; font-size: 11px; font-weight: 500; cursor: pointer; white-space: nowrap;">
                ${t('ws.addAndEnter')}
              </button>
            </div>
          ` : ''}
        ...
        ${(workspaces && workspaces.length > 0) ? `
          <div style="padding-top: 2px;">
            <div style="font-size: 11px; font-weight: 600; color: var(--dsw-alias-label-tertiary, #6b7280); margin-bottom: 4px;">${t('ws.registered', { n: workspaces.length })}</div>
      ...
      <div style="padding: 8px 16px; border-top: 1px solid var(--dsw-alias-border-l2, #e5e7eb); display: flex; align-items: center; justify-content: space-between; background: var(--dsw-alias-bg-layer-2, #f9fafb); flex-shrink: 0;">
        ...
        <button id="dsh-ws-cancel-btn" style="border: 1px solid var(--dsw-alias-border-l2, #d1d5db); background: var(--dsw-alias-bg-layer-1, #fff); color: var(--dsw-alias-label-primary, #111827); padding: 5px 14px; border-radius: 8px; font-size: 12px; cursor: pointer; font-weight: 500;">${t('btn.close')}</button>
      </div>
    `;
  ...
  async function loadDirectory(targetPath) {
    ...
    try {
      ...
      statusMessage = err.message || t('ws.readFail');
      isErrorMessage = true;
    } finally {
      isLoading = false;
      render();
  ...
  async function doSubmit(pathToRegister) {
    ...
    try {
      ...
      const res = raw?.value || raw;
      if (res && res.ok) {
        statusMessage = t('ws.selected', { title: res.title || p });
        isErrorMessage = false;
        workspaces = res.workspaces || [];
        render();
        ...
        statusMessage = res?.error || raw?.error || t('ws.addFail');
        isErrorMessage = true;
        render();
      }
    } catch (err) {
      statusMessage = err.message || t('ws.addErr');
      isErrorMessage = true;
      render();
    } finally {
      isSubmitting = false;
  ...
  if (typeof clientCtx?.locale?.subscribe === 'function') {
    unsubLocale = clientCtx.locale.subscribe(() => {
      if (document.getElementById('dsh-remote-workspace-modal')) render();
    });
  }
